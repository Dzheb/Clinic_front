  ...
  <div class="main">
    ...
    <div class="scroll-table">
      <table>
        ...
        <tbody>
          <tr v-for="(speciality, index) in specialitys" :key="speciality.id">
            <td>
              {{ speciality.speciality }}
            </td>
              <td>
              <button class="button" type="button" @click="updateSpeciality(index)">
            ...
            <td>
              ...
              >
              <svg width="16px" height="16px" viewBox="-10.5 0 141 141" fill="none"
    ...
    <div>
      <button class="button_submit button" type="submit" @click="addSpeciality()">
        Добавить
      </button>
    </div>
    <!-- update -->
    <div class="update_form_speciality" id="update_speciality" @click.self="closePopup">
      <form class="update_form_content_speciality">
        <h2>{{ form_header_speciality }}</h2>
        <div>Специализация врача: <input type="text" required v-model="speciality.speciality" /></div>      

        <div class="button_submit">
          <button type="submit" @click="sendSpeciality()">Отправить</button>
        </div>
      </form>
    </div>
  </div>
...
export default {
  ...
  methods: {
    deleteSpeciality(id) {
      //
      fetch('http://localhost:9000/speciality/' + id, {
        method: 'DELETE',
      })
        ...
        .then((data) => {
          if (data)
            fetch('http://localhost:9000/speciality')
              ...
              .then((appdata) => this.$emit('refreshSpecialitys', appdata));
        });
      //     
    },
    updateSpeciality(id) {
      this.add = false;
      this.form_header_speciality = 'Обновить информацию';
      const updateForm_speciality = document.querySelector('.update_form_speciality');
      updateForm_speciality.classList.toggle('show');
      this.speciality = this.specialitys[id];
    },
    addSpeciality() {
      this.add = true;
      ...
      const updateForm_speciality = document.querySelector('.update_form_speciality');
      updateForm_speciality.classList.toggle('show');
      this.form_header_speciality = 'Форма ввода специализации';
    },
    sendSpeciality() {
      var requestOptions;
      this.form_header_speciality = 'Обновление специализации';
      const json_object = Object.assign({}, this.speciality);
      ...
      if (this.add) {
        ...
        fetch('http://localhost:9000/speciality', requestOptions)
          .then((response) => response.json())
          .then((data) => {
            alert('Информация о специализации # ' + data + ' отправлена');
            fetch('http://http://localhost:9000/speciality')
              .then((response) => response.json())
              .then((specdata) => this.$emit('refreshSpeciality', specdata));
          });
      } else {
        ...
        fetch('http://localhost:9000/speciality/' + updateId, requestOptions)
          .then((response) => response.json())
          .then((data) => {
            alert('Информация о специализации # ' + data + ' отправлена');
            fetch('http://http://localhost:9000/speciality')
    ...
    closePopup() {
      window.onclick = function (event) {
        const updateForm_speciality = document.querySelector('.update_form_speciality');
        if (event.target == updateForm_speciality) {
          updateForm_speciality.classList.toggle('show');
        }
      };
    },
...
.update_form_content_speciality {
  display: flex;
  justify-content: center;
  align-items: center;
  flex-direction: column;
