<template>
  <div class="main">
    ...
    <!-- update -->
    <div
      class="update_form_speciality"
      id="update_speciality"
    ...
    >
      <form class="update_form_content_speciality">
        ...
        <div>
          Специализация врача:
          <input type="text" required v-model="speciality.speciality" />
        </div>
...
export default {
  ...
  methods: {
    ...
    updateSpeciality(id) {
      ...
      const updateForm_speciality = document.querySelector(
    ...
    sendSpeciality() {
      ...
      if (this.add) {
        ...
        fetch(this.url + 'speciality', requestOptions)
          .then((response) => response.json())
          .then((data) => {
        ...
        fetch(this.url + 'speciality/' + updateId, requestOptions)
          .then((response) => response.json())
          .then((data) => {
            alert('Информация о специализации # ' + data + ' отправлена');
          });
