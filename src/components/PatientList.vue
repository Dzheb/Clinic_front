<template>
  <div class="main">
    ...
    <div>
      <button class="button_submit button" type="submit" @click="addPatient()">
        Добавить
      </button>
    </div>
    <!-- update -->
...
export default {
  ...
  methods: {
    deletePatient(id) {
      //
      fetch(this.url+'appointment/patient/' + id, {
        method: 'DELETE',
      })
      ...
      //
      fetch(this.url+'patient/' + id, {
        method: 'DELETE',
      })
        .then((response) => response.ok)
        .then((data) => {
          if (data)
            fetch(this.url+'patient')
              .then((response) => response.json())
    ...
    sendPatient() {
      ...
      if (this.add) {
        ...
        fetch(this.url+'patient', requestOptions)
          .then((response) => response.json())
          .then((data) => {
        ...
        fetch(this.url+'patient/' + updateId, requestOptions)
          .then((response) => response.json())
          .then((data) => {
            alert('Информация о пациенте # ' + data + ' отправлена');
          });
