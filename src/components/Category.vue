<template>
  <div class="main">
        ...
        <tbody>
          <tr v-for="(category, index) in categorys" :key="category.id">
            <td>
              ...
              <td>
              <button class="button" type="button" @click="updateCategory(index)">
    ...
    <div>
      <button class="button_submit button" type="submit" @click="addCategory()">
        Добавить
      </button>
    </div>
    <!-- update -->
    <div class="update_form_category" id="update_category" @click.self="closePopup">
      <form class="update_form_content_category">
        <h2>{{ form_header_category }}</h2>
        <div>Категория врача: <input type="text" required v-model="category.category" /></div>      

        <div class="button_submit">
          <button type="submit" @click="sendCategory()">Отправить</button>
        </div>
      </form>
    </div>
  </div>
  ...
  methods: {
        ...
        .then((data) => {
          if (data)
            fetch('http://localhost:9000/category')
              ...
              .then((appdata) => this.$emit('refreshCategorys', appdata));
        });
      //     
    },
    updateCategory(id) {
      this.add = false;
      this.form_header_category = 'Обновить информацию';
      const updateForm_category = document.querySelector('.update_form_category');
      updateForm_category.classList.toggle('show');
      this.category = this.categorys[id];
    },
    addCategory() {
      this.add = true;
      ...
      const updateForm_category = document.querySelector('.update_form_category');
      updateForm_category.classList.toggle('show');
      this.form_header_category = 'Форма ввода категории';
    },
    sendCategory() {
      var requestOptions;
      this.form_header_category = 'Обновление категории';
      const json_object = Object.assign({}, this.category);
      ...
      if (this.add) {
        ...
        fetch('http://localhost:9000/category', requestOptions)
          .then((response) => response.json())
          .then((data) => {
            alert('Информация о категории # ' + data + ' отправлена');
            fetch('http://http://localhost:9000/category')
              .then((response) => response.json())
              .then((catdata) => this.$emit('refreshCategory', catdata));
          });
      } else {
        ...
        fetch('http://localhost:9000/category/' + updateId, requestOptions)
          .then((response) => response.json())
          .then((data) => {
            alert('Информация о категории # ' + data + ' отправлена');
            fetch('http://http://localhost:9000/category')
              .then((response) => response.json())
              .then((catdata) => this.$emit('refreshCategorys', catdata));
          });
      }
    ...
    closePopup() {
      window.onclick = function (event) {
        const updateForm_category = document.querySelector('.update_form_category');
        if (event.target == updateForm_category) {
          updateForm_category.classList.toggle('show');
        }
      };
    },
